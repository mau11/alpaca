...
Wolf.Doors = (function() {
    ...
    function spawn(level, x, y, type) {
        ...
        door.number = Math.ceil(Math.random() * 4);
    ...
    var chooseAnswer = _.throttle(function (door) {
       var match = window.answerHandler.chooseAnswer(document.getElementById('counter').innerHTML);
       if (match !== undefined) {
         door.answerChosen = true;
         if (match === true) {
            window.gameState.player.score += 100;
            window.gameState.player.health += 10;
         } else if (match === false) {
            if (window.gameState.player.score < 1000) {
              window.gameState.player.score = 0;
            } else {
              window.gameState.player.score -1000;
            }
            window.gameState.player.health -= 60;
            Wolf.Game.startDamageFlash(60);
         }
         if (window.gameState.player.health <= 0) {
            // dead
            Wolf.Game.notify("You have died");
            window.gameState.player.playstate = Wolf.ex_dead;
